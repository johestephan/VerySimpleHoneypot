...
def get_port_to_txt(port):
# https://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.xml
# based on url above, script will get description of port via CSV
     	with open('./service-names-port-numbers.csv','r') as csvfile:
		for line in csvfile:
			dataset = line.split(',')
			try:
				if int(dataset[1]) is port:
					return str(port) + " (" + dataset[0] + ")"
			except:
		                continue
		return  str(port) + " (UNKNOWN)"
